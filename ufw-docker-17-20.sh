...
function ufw-docker--check-install() {
	...
	*filter
	...
	-A DOCKER-USER -j RETURN -s 172.20.0.0/16
	...
	-A DOCKER-USER -j ufw-docker-logging-deny -p tcp -m tcp --tcp-flags FIN,SYN,RST,ACK SYN -d 172.20.0.0/16
	-A DOCKER-USER -j ufw-docker-logging-deny -p udp -m udp --dport 0:32767 -d 172.20.0.0/16
...
case "$ufw_action" in
	...
	service|raw-command|add-service-rule)
		...
		;;
#****************************************** INSTALL = ufw-docker--install = 420 str
