...
from app22.async_many_sql.schema_many_sql import (OrderCreateBody, OrderGetAllOrderbyQuery,
                                                  OrderResp, ProductResp, OrderGetQuery)

from app22.db_core.db_async import async_db
from app22.db_core.model.model_new_many_db import Order, Product


logFC = ConfigLogger.getLogger("FileStdout", "new_many_async_one")
...
async def insert_order(body: OrderCreateBody,
                       db: AsyncSession = Depends(async_db.get_db)):
...
async def get_order_filter_by(params: OrderGetQuery = Depends(),
                              db: AsyncSession = Depends(async_db.get_db)):
    # stmt: Select[tuple[Order]] = select(Order).filter_by(id=22)
    filter_where = {key: value for key, value in params.model_dump().items()
...
async def get_order_where(params: OrderGetQuery = Depends(),
                          db: AsyncSession = Depends(async_db.get_db)):
...
async def get_all_orders(variant: int = 1,
                         db: AsyncSession = Depends(async_db.get_db)):
    stmt: Select[tuple[Order]] = (select(Order).order_by(Order.id)
