...
from app22.async_many_sql.schema_many_sql import (OrderResp,
                                                  OrderGetQuery,
                                                  OrderUpdateBody)
...
logFC = ConfigLogger.getLogger("FileStdout", "new_many_async_two")
...
async def delete_order(params: OrderGetQuery = Depends(),
...
async def delete_list_order(params: OrderGetQuery = Depends(),
                            db: AsyncSession = Depends(async_db.get_db)):
...
async def update_order(body: OrderUpdateBody, params: OrderGetQuery = Depends(),
                       db: AsyncSession = Depends(async_db.get_db)):
...
async def update_list_order(body: OrderUpdateBody, params: OrderGetQuery = Depends(),
                             db: AsyncSession = Depends(async_db.get_db)):
    filter_where = [getattr(Order, key) == value
                    for key, value in params.model_dump(exclude_none=True).items()]
