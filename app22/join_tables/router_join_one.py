...
from app22.join_tables.schema_join import (CreateJoinPerson,
                                           GetJoinPerson,
                                           CreateJoinAddress,
                                           GetJoinAddress)
from app22.join_tables.schema_user_post import (CreateUser,
                                                CreatePost,
                                                GetUser,
                                                RespPost,
                                                GetPost)
...
async def get_person(db: AsyncSession = Depends(async_db.get_db)):
    ...
    stmt = (select(JoinPerson, JoinAddress)
            .join(JoinAddress, JoinAddress.addr_index == JoinPerson.link_addr))
    # stmt = (select(JoinPerson, JoinAddress)
...
async def create_person_addr(db: AsyncSession = Depends(async_db.get_db)):
    ...
    for idx, city, street in data_addr:
        addr = CreateJoinAddress(addr_index=idx, city=city, street=street, )
        await addrDB.add_record(addr, db)

    for name, surname, link_addr in data_pers:
        person = CreateJoinPerson(name=name, surname=surname, link_addr=link_addr, )
...
async def delete_person_addr(params_pers: GetJoinPerson = Depends(),
