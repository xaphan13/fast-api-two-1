...
from app22.db_core.model.model_reader_book import (Reader,
                                                   ...
                                                   Book,
                                                   Category)
from app22.reader_project.schema_reader import (CreateReader,
                                                SchemaReader,
                                                SchemaListBook,
                                                CreateListBook,
                                                CreateBook,
...
# ------------------------------------------------------------------------------
class ReaderAsyncCRUD(AsyncBaseCRUD[Reader, CreateReader, SchemaReader,
    ...
    async def get_reader_bookL_one(self, schema: ReaderType, db: AsyncSession) -> SqlType | None:
        query = select(self.model).where(*self._get_filter_attr(schema)) \
                .options(selectinload(self.model.book_lists))
...
# ------------------------------------------------------------------------------
class ListBookAsyncCRUD(AsyncBaseCRUD[ListBook, CreateListBook, SchemaListBook,
                                                SchemaListBook, SchemaListBook]):
    def get_model(self):
        return self.model
...
# ------------------------------------------------------------------------------
class BookAsyncCRUD(AsyncBaseCRUD[Book, CreateBook, SchemaBook,
                                        SchemaBook, SchemaBook]):

    async def add_book_to_list(self, schema: CreateType, db: AsyncSession,
                               listBooks: ListBook = None, commit: bool = True) -> SqlType:
...
# ------------------------------------------------------------------------------
class CategoryAsyncCRUD(AsyncBaseCRUD[Category, CreateCategory, SchemaCategory,
