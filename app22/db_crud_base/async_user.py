...
class UserAsyncCRUD(AsyncBaseCRUD[User, CreateUser, GetUser,
                                        CreateUser, GetUser]):

    # ==========================================================================
    # +++++++++ User -> posts=relationship[Post] - selectinload(Post) ++++++++++
    # --------------------------------------------------------------------------
    async def get_user_posts_one(self, schema: ReaderType,
                                 db: AsyncSession) -> SqlType | None:
        filter_attr = self._get_filter_attr(schema)
        query = select(self.model).where(*filter_attr)    \
    ...
    # --------------------------------------------------------------------------
    async def add_post_user(self, schema: CreatePost, user: User,
                            db: AsyncSession, commit: bool = True) -> Post:
