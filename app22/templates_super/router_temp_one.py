...
async def add_(body: CreateReader,
...
async def get_(params: SchemaReader = Depends(),
                        db: AsyncSession = Depends(async_db.get_db)):
...
async def update_(body: SchemaReader, params: SchemaReader = Depends(),
                           db: AsyncSession = Depends(async_db.get_db)):
    if body is not None:
...
async def delete_(params: SchemaReader = Depends(),
                            db: AsyncSession = Depends(async_db.get_db)):
