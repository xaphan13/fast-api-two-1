...
class ListBookAssociation(Base):
    ...
    __table_args__ = (
        UniqueConstraint('list_id', 'book_id', name='idx_unique_list_book'),
    )
    ...
    list_book = relationship('ListBook', back_populates='book_associations')

    book = relationship('Book', back_populates='list_associations')

    def __repr__(self):
        return (f"ListBookAssociation(id={self.id}, time_add={self.time_add}, "
...
class BookCategoryAssociation(Base):
    ...
    __table_args__ = (
        UniqueConstraint('category_id', 'book_id', name='idx_unique_category_book'),
    )
    ...
    def __repr__(self):
        return (f"BookCategoryAssociation(id={self.id}, "
                f"book_id={self.book_id}, category_id={self.category_id})")
