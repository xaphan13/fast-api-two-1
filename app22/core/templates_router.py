...
from app22.core.templates_schema import Resp, Create, Get
...
async def get_(params: Get = Depends(),
               db: AsyncSession = Depends(async_db.get_db)):
...
async def update_(body: Create, params: Get = Depends(),
                  db: AsyncSession = Depends(async_db.get_db)):
    raise HTTPException(status_code=422, detail=f" with {params} not found")
...
async def delete_(params: Get = Depends(),
                  db: AsyncSession = Depends(async_db.get_db)):
    raise HTTPException(status_code=422, detail=f" with {params} not found")
