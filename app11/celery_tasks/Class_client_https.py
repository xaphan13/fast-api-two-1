...
class ClientHTTPS:
    ...
    # ------------------------------------------------------------------
    def get_req_create(self, path: str, params: Optional[dict] = None,
                                        callback: Optional[Callable] = None) -> Task[RespServer]:
        paramsR = {} if params is None else params
    ...
    # ------------------------------------------------------------------
    def post_req_create(self, path: str, payload: dict, params: Optional[dict] = None,
                                         callback: Optional[Callable] = None) -> Task[RespServer]:
        paramsR = {} if params is None else params
        url = f"{self.protocol}{self.server}{path}"
        task: Task[RespServer] = asyncio.create_task(self.post_req_send(url, self.headers, body=payload, params=paramsR))
        if callback is not None:
            task.add_done_callback(callback)
        return task
# ********************************************************************************************
...
async def main_weather(city: str, appid: str) -> RespServer:
